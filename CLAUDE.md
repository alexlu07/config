...
- **neo-tree** (`lua/plugins/neo-tree.lua`): `<leader>f` toggles a floating tree (conform's format-buffer map moved to `<leader>F` to free it). Custom mappings: `/` and `?` are unbound in the tree so normal vim search works (n/N to jump); `O` = help; `F` = collapse everything except the path to the cursor (custom `fold_all_others`); `z`/`Z` = close/expand all subnodes of the node under the cursor (instead of the whole tree).
