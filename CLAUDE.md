...
Editor is Neovim, configured via `templates/nvim/` (linked as a directory to `~/.config/nvim`). The config is a customized [kickstart.nvim](https://github.com/nvim-lua/kickstart.nvim) using the built-in `vim.pack` plugin manager (requires nvim 0.12+), shipping Telescope, Treesitter, LSP + completion, gitsigns, which-key, neo-tree, diffview, fugitive. Local customizations are marked with comments and keep the upstream lines commented out beside them.
...
- **neo-tree** (`lua/plugins/neo-tree.lua`): `<leader>f` toggles a floating tree (conform's format-buffer map moved to `<leader>F` to free it). Custom mappings: `/` `?` `n` `N` search the whole tree rather than the rendered buffer (a vim `/` can only match visible rows, so collapsed directories are invisible to it). Typing the pattern uses a `getcharstr` loop that only sets `@/` + `hlsearch`, so on-screen matches highlight while **nothing** expands or gets scanned until `<CR>`. On accept, `filter_external.find_files` enumerates matches (its dotfile/gitignore filtering agrees with what the tree displays), they're sorted into render order (`display_order`: directories before files), and each jump expands **only** that match's ancestors via `focus_node` (already-loaded nodes) or `navigate(…, path_to_reveal)` (never-scanned paths). Every jump — `/` and `n`/`N` alike — is computed from the node under the cursor *at that moment* (there is deliberately no stored "current match" index), so moving around the tree by hand and then pressing `n` continues from where you are. Only the direction is remembered, so `n` repeats it and `N` reverses, as in vim; walking off either end wraps and says so. Directory matches arrive with a trailing slash that must be stripped or they never match a node id. With no search active `n`/`N` fall back to builtin. Capped by `search_limit` (50), which warns when hit; `O` = help; `F` = collapse everything except the path to the cursor (custom `fold_all_others`); `z`/`Z` = close/expand all subnodes of the node under the cursor (instead of the whole tree).
- **diffview** (`lua/plugins/diffview.lua`): git diff / merge tool / file history. Global maps under `<leader>d`, where case is scope (lowercase = current file, uppercase = whole repo) and an `l` infix means "compare against the working tree" instead of against each commit's own parent: `dd` toggle diffview, `dh`/`dH` file/repo history (`x` mode: history of the selected lines), `dm` merge base with the remote's default branch vs HEAD (committed only), `dlh`/`dlH` file/repo history with `--base=LOCAL`, `dlm` the default branch vs the working tree (uncommitted + untracked included). The default branch is resolved from `origin/HEAD`, not hardcoded. Defaults kept except `enhanced_diff_hl = true` and `diff3_mixed` for the merge tool. In-view keymaps are diffview's defaults (see `g?`); both panels additionally get `O` = help, `C` = collapse fold, `z`/`Z` = collapse/expand all, matching the neo-tree bindings, and `<leader>e` = `focus_entry` so it toggles focus with the `view` context's `<leader>e`.
- **fugitive** (`lua/plugins/fugitive.lua`): git porcelain for what diffview deliberately doesn't do. Diffview has no commit action, so `<leader>gc` = `:Git commit` (opens COMMIT_EDITMSG in this instance rather than a nested nvim in a terminal buffer). A `FileType gitcommit` autocmd relocates that buffer into a centered float to match neo-tree; it no-ops when the commit buffer is the only window (git running nvim as `$EDITOR`), since nvim can't close the last non-floating window.
