...
Editor is Neovim, configured via `templates/nvim/` (linked as a directory to `~/.config/nvim`). The config is a customized [kickstart.nvim](https://github.com/nvim-lua/kickstart.nvim) using the built-in `vim.pack` plugin manager (requires nvim 0.12+), shipping Telescope, Treesitter, LSP + completion, gitsigns, which-key, neo-tree, diffview. Local customizations are marked with comments and keep the upstream lines commented out beside them.
